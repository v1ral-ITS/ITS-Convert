...
class DartEmitter:
    ...
    def _n(self, node, i):
        ...
        if isinstance(node, Input):
            return [f"{p}stdout.write('{node.prompt}');",
                    f"var {node.name} = stdin.readLineSync() ?? '';"]
        if isinstance(node, Exit): return [f"{p}exit({node.code});"]
        if isinstance(node, If): return self._if(node, i)
        if isinstance(node, For):
            return [f"{p}for (var {node.var} in {self._v(node.iterable)}) {{"] + self._body(node.body, i+1) + [f"{p}}}"]
        ...
        if isinstance(node, ForEnumerate):
            return [f"{p}for (({node.index_var}, {node.value_var}) in {self._v(node.iterable)}.indexed) {{"] + self._body(node.body, i+1) + [f"{p}}}"]
        ...
        if isinstance(node, EnvVar):
            if node.action == "get" and node.result_name: return [f"{p}var {node.result_name} = Platform.environment['{node.name}'];"]
        ...
        if isinstance(node, ClassDef):
            ...
            for field in node.fields:
                ...
                lines.append(f"{p}  dynamic {field.name}{val};")
    ...
    def _list(self, n, p):
        ...
        if n.action == "len" and n.result_name: return [f"{p}var {n.result_name} = {nm}.length;"]
        if n.action == "join" and n.result_name and n.value: return [f"{p}var {n.result_name} = {nm}.join({self._v(n.value)});"]
        if n.action == "sort" and n.result_name: return [f"{p}var {n.result_name} = List.from({nm})..sort();"]
    ...
    def _dict(self, n, p):
        nm = n.name or "map"
        if n.action == "create":
            if not n.items: return [f"{p}var {nm} = <String, dynamic>{{}};"]
            pairs = [f"{self._v(k)}: {self._v(v)}" for k, v in n.items]
            return [f"{p}var {nm} = <String, dynamic>{{{', '.join(pairs)}}};"]
    ...
    def _file(self, n, p):
        path = self._v(n.path)
        if n.op == "read" and n.name: return [f"{p}var {n.name} = File({path}).readAsStringSync();"]
        if n.op == "write" and n.content: return [f"{p}File({path}).writeAsStringSync({self._v(n.content)});"]
        if n.op == "append" and n.content: return [f"{p}File({path}).writeAsStringSync({self._v(n.content)}, mode: FileMode.append);"]
        if n.op == "exists": return [f"{p}var {n.name or '_ex'} = File({path}).existsSync();"]
        if n.op == "delete": return [f"{p}File({path}).deleteSync();"]
        if n.op == "mkdir": return [f"{p}Directory({path}).createSync(recursive: true);"]
...
def emit_dart(ir: ScriptIR) -> str: return DartEmitter().emit(ir)
