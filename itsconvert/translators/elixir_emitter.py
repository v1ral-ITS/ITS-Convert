...
class ElixirEmitter:
    def emit(self, ir: ScriptIR) -> str:
        ...
        lines: list[str] = ["defmodule Main do", ""]
        for fn in fns:
            lines.extend(self._fn(fn, 1))
            lines.append("")
        lines.append("  def main(_args) do")
        for node in other:
            lines.extend(self._n(node, 2))
        lines.append("  end")
        lines.append("end")
        lines.append("")
        lines.append("Main.main(System.argv())")
        return "\n".join(lines) + "\n"
    ...
    def _n(self, node, i):
        ...
        if isinstance(node, Print):
            if not node.values: return [f'{p}IO.puts("")']
            args = " <> \" \" <> ".join(f"to_string({self._v(v)})" for v in node.values)
            fn = "IO.write" if node.end == "" else "IO.puts"
            return [f"{p}{fn}({args})"]
        if isinstance(node, Input):
            if node.prompt: return [f'{p}IO.write("{node.prompt}")', f"{p}{node.name} = IO.gets(\"\") |> String.trim()"]
            return [f"{p}{node.name} = IO.gets(\"\") |> String.trim()"]
        ...
        if isinstance(node, ForRange):
            ...
            return [f"{p}Enum.each({s}..{e}-1, fn {node.var} ->"] + self._body(node.body, i+1) + [f"{p}end)"]
        if isinstance(node, ForEnumerate):
            return [f"{p}Enum.each(Enum.with_index({self._v(node.iterable)}), fn {{{node.value_var}, {node.index_var}}} ->"] + self._body(node.body, i+1) + [f"{p}end)"]
        if isinstance(node, ForKeys):
            return [f"{p}Enum.each(Map.keys({self._v(node.dict_value)}), fn {node.var} ->"] + self._body(node.body, i+1) + [f"{p}end)"]
        if isinstance(node, For):
            return [f"{p}Enum.each({self._v(node.iterable)}, fn {node.var} ->"] + self._body(node.body, i+1) + [f"{p}end)"]
        if isinstance(node, While):
            return [f"{p}# while {self._cond(node.condition)}:"] + self._body(node.body, i+1)
        if isinstance(node, Break): return [f"{p}# break (use Enum.reduce_while instead)"]
        if isinstance(node, Continue): return [f"{p}# continue (use Enum.reduce_while instead)"]
        if isinstance(node, Pass): return [f"{p}:ok"]
        if isinstance(node, Return): return [f"{p}{self._v(node.value) if node.value else ':ok'}"]
        ...
        if isinstance(node, EnvVar):
            ...
            if node.action == "set": return [f'{p}System.put_env("{node.name}", {self._v(node.value)})']
            if node.action == "delete": return [f'{p}System.delete_env("{node.name}")']
            return [f"{p}# env: {node.action}"]
        if isinstance(node, Argv):
            if node.action == "all" and node.name: return [f"{p}{node.name} = System.argv()"]
            if node.action == "nth" and node.index is not None and node.name: return [f"{p}{node.name} = Enum.at(System.argv(), {node.index})"]
        ...
        if isinstance(node, Raise):
            ...
            return [f"{p}raise RuntimeError, {msg}"]
    ...
    def _fn(self, n, i):
        ...
        lines = [f"{p}def {n.name}({params}) do"]
    ...
    def _list(self, n, p):
        ...
        if n.action == "append": return [f"{p}{nm} = {nm} ++ [{self._v(n.value)}]"]
        if n.action == "len" and n.result_name: return [f"{p}{n.result_name} = length({nm})"]
        if n.action == "sort" and n.result_name: return [f"{p}{n.result_name} = Enum.sort({nm})"]
        if n.action == "contains" and n.value and n.result_name: return [f"{p}{n.result_name} = Enum.member?({nm}, {self._v(n.value)})"]
        if n.action == "join" and n.result_name and n.value: return [f"{p}{n.result_name} = Enum.join({nm}, {self._v(n.value)})"]
    ...
    def _dict(self, n, p):
        ...
        if n.action == "get" and n.key and n.result_name: return [f"{p}{n.result_name} = Map.get({nm}, {self._v(n.key)})"]
        if n.action == "set" and n.key: return [f"{p}{nm} = Map.put({nm}, {self._v(n.key)}, {self._v(n.value)})"]
        if n.action == "delete" and n.key: return [f"{p}{nm} = Map.delete({nm}, {self._v(n.key)})"]
        if n.action == "keys" and n.result_name: return [f"{p}{n.result_name} = Map.keys({nm})"]
        if n.action == "values" and n.result_name: return [f"{p}{n.result_name} = Map.values({nm})"]
        if n.action == "len" and n.result_name: return [f"{p}{n.result_name} = map_size({nm})"]
        return [f"{p}# dict: {n.action}"]

    def _strop(self, n, p):
        if not n.operands: return [f"{p}# string_op: {n.op}"]
        base = self._v(n.operands[0])
        if n.op == "upper" and n.name: return [f'{p}{n.name} = String.upcase({base})']
        if n.op == "lower" and n.name: return [f'{p}{n.name} = String.downcase({base})']
        if n.op == "strip" and n.name: return [f'{p}{n.name} = String.trim({base})']
        if n.op == "len" and n.name: return [f'{p}{n.name} = String.length({base})']
        if n.op == "replace" and len(n.operands) >= 3 and n.name:
            return [f'{p}{n.name} = String.replace({base}, {self._v(n.operands[1])}, {self._v(n.operands[2])})']
        if n.op == "split" and len(n.operands) >= 2 and n.name:
            return [f'{p}{n.name} = String.split({base}, {self._v(n.operands[1])})']
        if n.op == "contains" and len(n.operands) >= 2 and n.name:
            return [f'{p}{n.name} = String.contains?({base}, {self._v(n.operands[1])})']
        if n.op == "startswith" and len(n.operands) >= 2 and n.name:
            return [f'{p}{n.name} = String.starts_with?({base}, {self._v(n.operands[1])})']
        if n.op == "endswith" and len(n.operands) >= 2 and n.name:
            return [f'{p}{n.name} = String.ends_with?({base}, {self._v(n.operands[1])})']
    ...
    def _file(self, n, p):
        ...
        if n.op == "append" and n.content:
            return [f"{p}File.write!({path}, {self._v(n.content)}, [:append])"]
        if n.op == "exists" and n.name: return [f"{p}{n.name} = File.exists?({path})"]
        if n.op == "mkdir": return [f"{p}File.mkdir_p!({path})"]
        if n.op == "delete": return [f"{p}File.rm!({path})"]
    ...
    def _v(self, v):
        ...
        if v.kind == "fstring" and v.parts:
            parts = [f"#{{to_string({self._v(p)})}}" if p.kind != "string" else str(p.value) for p in v.parts]
            return '"' + "".join(parts) + '"'
        ...
        if v.kind == "subscript" and v.parts and len(v.parts) >= 2:
            return f"Enum.at({self._v(v.parts[0])}, {self._v(v.parts[1])})"
        return repr(v.value)
...
def emit_elixir(ir: ScriptIR) -> str: return ElixirEmitter().emit(ir)
